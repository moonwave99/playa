...
import type { GroupCreate, GroupUpdate, HasId, PaginationParams, Artist, GroupWithArtists } from '@/types/types';
...
export async function getGroups({ take = 50 }: PaginationParams) {
  ...
  return withEntityType(
    results.map(
      (x: GroupWithArtists) => ({ ...x, artists: withEntityType(x.artists, 'artist') })), 'group'
  );
}
...
export async function getGroup(id: number) {
  ...
  return result
    ? withEntityType({ ...result, artists: withEntityType(result.artists, 'artist') }, 'group') : null;
}
...
export async function addArtistsToGroup(id: number, artists: Artist[]) {
  const result = await prisma.group.update({
    where: {
      id
    },
